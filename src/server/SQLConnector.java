...
//            statement.executeUpdate("INSERT INTO table_user VALUES (NULL, 'First_User', 'test@email.ru');"); // Вставляем значение в таблицу.
            select = statement.executeQuery("SELECT * FROM table_user WHERE id = 1;"); // Создаём запрос в базу данных
            while(select.next()){
...
//        select = statement.executeQuery(qwery);
    ...
    public boolean autUser2(String login, String password){
        String qwery2 = ("SELECT * FROM table_user WHERE name = ?;"); // Сборная строка, где за место знака вопроса подставляется значение
